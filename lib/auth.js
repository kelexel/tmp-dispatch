...
// 	// if no dispatcherType is passed, assume we're using the "public" dispatcherType
		...
		var authCallback =  function(statusCode, result){	
			...
			if (statusCode != 200) return callback('Error invalid token', false);
		...
		request.getJSON(options, authCallback);
	},
	populate: function(options) {
		...
		// session.eventLock = options.eventLock;
		// session.eventTester = options.eventTester;
