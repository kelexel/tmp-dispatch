...
var output = function(buffer, indent) {
	...
	if (type(buffer) != 'object' && type(buffer) != 'array')
		...
		// console.log('buffer has type', type(buffer));
		// return;
		// return;
		object.each(buffer, function(v, k){
...
var commands = {
	...
	'ls': {
		'command': function(args) {
			...
			if (args[1]) {
				// console.log('oui')
				if (['manager', 'display', 'public', 'shell'].indexOf(args[1]) == -1) {
	...
	'rp': {
		'command': function(args) {
			...
			if (args[2]) {
				if (['manager', 'display', 'public', 'shell'].indexOf(args[2]) == -1) {
					return logger.error('Invalid dispatcher '+args[1]);
					}
			} else {
				args[2] = '/public';
...
stdin.addListener("data", function(d) {
	...
	// switch (cmd) {
		...
		// default:
			object.each(commands, function(att, cmd) {
