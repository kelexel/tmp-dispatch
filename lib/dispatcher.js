...
console.log('payload', payload);
		if (payload.command == 'setSlide') this.sessionUpdateSlide(io, socket, payload);
...
// Server emitter to all interfaces of a given event
	...
	pollContent: function(session, interface, callback) {

		// if (!this._polling[session.slide.id]) return;

		var options = {
			host: _env.get('backendHostname').toString(),
			port: _env.get('backendPort').toString(),
			path: '/h/'+session.eventHash+'/'+interface+'/embed/'+session.slide.id+'?refresh=true&behaviour=all',
			method: 'GET',
			headers: {
		...
		logger.debug('polling content for slide', session.slide.id);
		var backend = require(__dirname+'/backend.js');	
		backend.getJSON(options, callback);
