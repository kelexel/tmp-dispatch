...
module.exports = prime({
	...
	constructor: function(io, server) {
		...
		emitter.on('dispatch'+_dispatcherTyper+'Options', this.serverPropagateEvent.bind(this));
...
// Socket.io related
	...
	socketSetOptionsListener: function(server, socket, payload) {
		...
		var callback = function(data) {
			...
			this.serverPropagateToDispatchers(server, 'option', message);
		}.bind(this);

		backend.asyncSendSlideOption(socket, payload, callback);
	},

	socketSendCommandListener: function(server, socket, payload) {
		this.debug('listener setOption received', payload);
		var session = socket.handshake._session;
		var backend = require(__dirname+'/backend.js');

		// var message = payload;
		this.serverPropagateToDispatchers(server, 'command', payload);
	},
...
// Server emitter to all interfaces of a given event
	serverPropagateToDispatchers: function(server, eventType, payload) {
		...
		_(payload.dispatchers).each(function(dispatcher) {
			...
			emitter.emit('dispatch'+dispatcher+'Options', eventType, payload);
		}, this);
		return this;
	},
	serverPropagateEvent: function(eventType, payload) {
		this.debug(this._dispatcherTyper+' serverPropagateEvent', eventType, payload);
		// 	var message = {
