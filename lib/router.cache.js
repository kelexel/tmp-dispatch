...
// > db.cache.update({clients: 'M3YHPBHJd_vPaeHfaqyL'}, {$pull: {clients: 'M3YHPBHJd_vPaeHfaqyL'}}, {multi: true});
				...
				async.waterfall([
					...
					function(io, eid, sid, clients, endpoint, statusCode, response, callback){
						...
						if (statusCode == '200') {
							...
							this.cachePurgeSlide(eid, sid, function(err, res) {
								if (err) console.log('have an error in purge', err);
								callback(null);
							});
							return;
						} else {
							console.log('got another statusCode !!');
	...
	cacheStore: function(eid, sid, url, response, callback) {
		...
		// if (response.data.options) console.log(response.data.options.displayType)
		var newHash = crypto.createHash('md5').update(JSON.stringify(response)).digest('hex');
		// console.log('newhas is '+newHash);

		if (!this._cacheHashes) this._cacheHashes = {};
		if (!this._cacheHashes[eid]) this._cacheHashes[eid] = {};

		if (this._cacheHashes[eid][sid] != newHash) {
			...
			this._cacheHashes[eid][sid] = newHash;
		} else {
			// console.log('cache no change')
