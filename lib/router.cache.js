...
module.exports = {
	...
	cachePurge: function(io, parentCallback) {
		async.waterfall([
			function(callback) {
				this._cacheClient.collection('cache').find({}, ['clients']).toArray(callback);
			}.bind(this),
			...
			function(toTrash, callback) {
				async.each(toTrash,
					...
					function(err) { 
						if (err) { 
							console.log('woops'); callback(err);
						}
					}
				);
				callback(false, true);
...
// > db.cache.update({clients: 'M3YHPBHJd_vPaeHfaqyL'}, {$pull: {clients: 'M3YHPBHJd_vPaeHfaqyL'}}, {multi: true});
	...
	cacheFetchManagerSlideContent: function(io, parentCallback) {
		...
		this._cacheClient.collection('cache').find().toArray(function(err, res) {
			...
			async.series([
				function(callback) {
					...
					res.forEach(function(item) {
						...
						if (item.clients && type(item.clients) == 'array' && item.clients.length > 0 && ['cloud', 'conference', 'quiz', 'poll'].indexOf(item.type) != '-1') {
							...
							cb = function(io, eid, sid, endpoint, response) {
								propagateCallback = function(err, changed) {
									...
									if (changed) {
										...
										// this.cachePropagateToClients(io, eid, sid, '/display', item.clients, response);
									} else {
										// console.log('cache not changed for '+sid)
									}
									// console.log('c is '+c+' and max is '+max)
									if (c == max) callback(null, true);
								}.bind(this);
				...
				if (res) parentCallback(null, true);
			});
		}.bind(this));
	},
