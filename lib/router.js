...
// console.log(socket.manager.transports[socket.id].open);
		...
		// } else {
			// console.log('running shell attach')
			// this.shellAttach(io, socket);
			// logger.debug('CID: '+this._cid+' | _masterClientRegister: registered new '+socketEndpoint+' socketId '+socket.id);
		// }
		this.helperPropagateEmitterEvent('master', 'newClient', {'socketId': socket.id, 'endpoint': socketEndpoint});
