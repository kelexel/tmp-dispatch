...
module.exports = {
	...
	_shellEventEmitterListener: function(io, socket, eventPayload) {
		...
		switch(eventPayload.cmd) {
			...
			case 'listSockets':
				...
				if (sockets && type(sockets) == 'array' && sockets.length > 0) {
					sockets.forEach(function(sid) {
						s = this.helperGetSocketById(io, sid)
			...
			break;
			case 'listCache':
			console.log(this._cid+' iopiopiop')
			console.log(this._cachedSlides);
			var response = this._cachedSlides.valueOf();
			console.log(response)
				socket.emit('shellResponse', {'status': 'ok', 'buffer':  {'cid': this._cid, 'response': response}});
			break;
			case 'listPollings':
